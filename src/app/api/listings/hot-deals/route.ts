...
import { getHotDealPropertyListingsPage } from "@/features/products/server-data";
...
/** Public hot-deal listings — paginated (default 6 per page). */
export const GET = compose(withHandler)(async (request: NextRequest) => {
  ...
  return ok(await getHotDealPropertyListingsPage(page, pageSize));
});
