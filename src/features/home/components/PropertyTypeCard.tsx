  ...
  return (
    <Link
      ...
      className={`group flex flex-col items-center justify-center text-center transition-transform duration-300 hover:-translate-y-1 ${
        compact
          ? "min-h-[5.9rem] gap-1.5 rounded-none bg-transparent px-1 py-3 shadow-none sm:gap-2 sm:px-2"
    ...
    >
      <Icon
        className={`text-[var(--brand-blue)] transition-transform duration-300 group-hover:scale-105 ${
          compact ? "h-7 w-7 sm:h-9 sm:w-9" : "h-14 w-14 md:h-10 md:w-10 lg:h-16 lg:w-16"
        }`}
      />
      <span
        className={`font-bold text-[var(--brand-navy)] ${
          compact
            ? "text-[0.625rem] leading-tight min-[390px]:text-[0.7rem] sm:text-[0.95rem]"
